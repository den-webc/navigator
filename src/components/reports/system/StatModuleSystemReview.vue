		...
		<div class="c-container">
			<div class="c-container__left">
				...
				<div class="c-block mt-1" v-if="issueList.length > 0">
					<div class="c-block__header">
						Осмотры, обнаружившие проблему, но не повлекшие назначение работ
					</div>
			...
			<div class="c-container__right" v-if="chartIssueNum.chartData.labels.length > 0 || chartRaised.chartData.labels.length > 0">
				<div class="c-block" v-if="chartIssueNum.chartData.labels.length > 0">
					<div class="c-block__header">
						Количество осмотров, выявивших проблему
					</div>
					<div class="c-block__body">
						<div class="c-chart">
				...
				<div class="c-block" v-if="chartRaised.chartData.labels.length > 0">
					<div class="c-block__header">
						Количество работ после осмотров с ненормативным состоянием
					</div>
					<div class="c-block__body">
						<div class="c-chart">
							<div class="c-chart__container">
								<PieChart :chartData="chartRaised.chartData" :options="chartRaised.options"/>
							</div>
						</div>
					</div>
					<div class="c-block__footer">
						<div class="c-legend">
							<div class="c-legend__item" v-for="(label, index) in chartRaised.chartData.labels" :key="index">
								<div class="c-legend__badge" :style="{backgroundColor: chartRaised.chartData.datasets[0].backgroundColor[index]}"></div> 
								{{label}}
							</div>
						</div>
					</div>
	    ...
	    methods: {
	    	...
	    	async loadResult () {
	        	...
	        	const param = this.buildfilter();
	      		...
	      		this.result = await this.$store.dispatch('getAnaliticsSystemModuleReviewResult', {house_id: this.house_id, module_id: this.module_id, data: param});
	      		
	      		this.issueList = await this.$store.dispatch('getAnaliticsSystemModuleReviewIssuelist', {house_id: this.house_id, module_id: this.module_id, data: param});

	      		const issueNum = await this.$store.dispatch('getAnaliticsSystemModuleReviewIssueNum', {house_id: this.house_id, module_id: this.module_id, data: param});
	      		...
	      		const raised = await this.$store.dispatch('getAnaliticsSystemModuleReviewRaised', {house_id: this.house_id, module_id: this.module_id, data: param});

	      		if(raised.workRaised + raised.workNotRaised > 0) {

	      			this.chartRaised.chartData.labels = ['Работа была назначена', 'Работа не была назначена'];
	      			this.chartRaised.chartData.datasets[0].data = [raised.workRaised, raised.workNotRaised];
	      			this.chartRaised.chartData.datasets[0].percentage = [raised.workRaisedP, raised.workNotRaisedP];
	      			this.chartRaised.chartData.datasets[0].backgroundColor = ['#008dd2', '#aec1cc'];
